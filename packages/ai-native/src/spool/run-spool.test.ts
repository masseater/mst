...
describe("退避の記録", { timeout: 30_000 }, () => {
  ...
  test("エスケープ列は記録から除去され可視文字は残る", async () => {
    const script = String.raw`process.stdout.write("\u001b[31mred\u001b[0m plain\n");`;
    const recordedRun = await runRecorded(["--", node, "-e", script]);
...
describe("終了コードと結末", { timeout: 30_000 }, () => {
  ...
  test("シグナルで死んだ子は 128+signum で返り記録はそこまで残る", async () => {
    const script = String.raw`process.stdout.write("before signal\n", () => process.kill(process.pid, "SIGKILL"));`;
    const recordedRun = await runRecorded(["--", node, "-e", script]);
...
describe("縫い目と既定値", { timeout: 30_000 }, () => {
