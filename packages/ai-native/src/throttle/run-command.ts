...
const ALREADY_RELEASED_CODES: ReadonlySet<string> = new Set(["ERELEASED", "ENOTACQUIRED"]);

const releaseUnlessAlreadyGone = async (release: () => Promise<void>): Promise<void> => {
  try {
    await release();
  } catch (staleRelease) {
    if (failedWithCode(staleRelease, ALREADY_RELEASED_CODES)) return;
    throw staleRelease;
  }
};
...
export const runWithSlot = async (invocation: Invocation, hold: SlotHold): Promise<number> => {
  ...
  await releaseUnlessAlreadyGone(hold.release);
