...
const RELEASE_VERSION_PATTERN =
  /^v[0-9]+(?:\.[0-9]+)*(?:-[0-9A-Za-z]+(?:[.-][0-9A-Za-z]+)*)?(?:\+[0-9A-Za-z]+(?:[.-][0-9A-Za-z]+)*)?$/u;
...
export const unpinnedActionRefs = ({
  ...
  referenceEntries({ document, config }).flatMap((actionEntry) => {
    ...
    const annotation = trailingComment(actionEntry.value);
    if (annotation !== null && RELEASE_VERSION_PATTERN.test(annotation.trim())) return [];
