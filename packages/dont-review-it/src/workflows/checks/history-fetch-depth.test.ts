...
describe("unboundedHistoryFetches", () => {
  ...
  describe("custom actions that happen to accept an input named fetch-depth", () => {
    const it = test.extend("historyFetchProblemsByAction", () =>
      ["acme/custom-checkout", "actions/checkout-helper"].map((action) =>
        unboundedHistoryFetches({
          document: parseWorkflowDocument({
            relativePath: ".github/workflows/ci.yml",
            source: `jobs:\n  build:\n    steps:\n      - uses: ${action}@v1\n        with:\n          fetch-depth: 0\n`,
    ...
    it("does not assign checkout semantics to another action's input", ({
      historyFetchProblemsByAction,
    }) => {
      expect(historyFetchProblemsByAction).toStrictEqual([[], []]);
    });
  });

  describe("a checkout reference with different letter casing", () => {
    const it = test.extend("historyFetchProblems", () =>
      unboundedHistoryFetches({
        document: parseWorkflowDocument({
          relativePath: ".github/workflows/ci.yml",
    ...
    it("applies checkout semantics to the same GitHub repository", ({ historyFetchProblems }) => {
      expect(historyFetchProblems).toStrictEqual([
        {
          file: ".github/workflows/ci.yml",
          line: 6,
