...
import { composedPrefixOf, entryBodyOf, type EntryCompositionConfig } from "./config.ts";
...
export type EntryFinding =
  ...
  | { readonly kind: "non-string-entry"; readonly entryName: string; readonly node: Node }
  | { readonly kind: "missing-body"; readonly entryName: string; readonly node: Node }
  | {
...
export const entryFindingsIn = ({
  ...
  return layer.entryNames.flatMap((entryName): readonly EntryFinding[] => {
    ...
    if (typeof scriptNode.value !== "string") {
      return [{ kind: "non-string-entry", entryName, node: scriptNode }];
    }
    const scriptText = scriptNode.value;
    if (entryBodyOf({ value: scriptText, layer, config }).trim() === "") {
      return [{ kind: "missing-body", entryName, node: scriptNode }];
    }
...
const problemOf = ({
  ...
  if (finding.kind === "non-string-entry") {
    return {
      file: manifest.relativePath,
      line: lineAt({ source: manifest.source, offset: finding.node.offset }),
      message: `The "${finding.entryName}" script must not be a non-string value. Replace it with a string that starts with the required prefix "${prefix}", then write the intended command after that prefix.`,
    };
  }
  if (finding.kind === "missing-body") {
    return {
      file: manifest.relativePath,
      line: lineAt({ source: manifest.source, offset: finding.node.offset }),
      message: `The "${finding.entryName}" script must not end before its command body. Keep the required prefix "${prefix}" and write the intended command after it.`,
    };
  }
  return {
    file: manifest.relativePath,
