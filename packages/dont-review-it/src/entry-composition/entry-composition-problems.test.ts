...
describe("entryCompositionProblems", () => {
  ...
  test("reports a non-string script without inventing its command body", () => {
    const repositoryRoot = repositoryWith({
    ...
    expect(problems[0]?.message).toBe(
      `The "guard" script must not be a non-string value. Replace it with a string that starts with the required prefix "${ROOT_PREFIX}", then write the intended command after that prefix.`,
    );
  });

  test.each(["", ROOT_PREFIX, "spool -- ", "spool --"])(
    "reports a script whose command body is missing: %s",
    (guard) => {
      const repositoryRoot = repositoryWith({
        "package.json": JSON.stringify({ scripts: { guard } }),
      });
