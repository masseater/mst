...
describe("entryCompositionProblems", () => {
  ...
  describe("a script whose value is not a string", () => {
    ...
    it("reports the value by its actual type", ({ report }) => {
      expect(report).toStrictEqual({
        problems: [
          {
            file: "package.json",
            line: 1,
            message: `The "guard" script must not be a non-string value. Replace it with a string that starts with the required prefix "${ROOT_PREFIX}", then write the intended command after that prefix.`,
          },
        ],
        failures: [],
        scanned: 1,
  ...
  describe("a script whose required prefix has no command body", () => {
    const it = test.extend("report", ({}, { onCleanup }) => {
      const repositoryRoot = mkdtempSync(join(tmpdir(), "entry-composition-problems-"));
      onCleanup(() => {
        rmSync(repositoryRoot, { recursive: true, force: true });
      ...
      writeFileSync(
        join(repositoryRoot, "package.json"),
        `{ "scripts": { "guard": "${ROOT_PREFIX}" } }`,
        "utf8",
      );
      return entryCompositionProblems({ repositoryRoot, config: defaultEntryCompositionConfig });
    });

    it("requires a command after the prefix", ({ report }) => {
      expect(report).toStrictEqual({
        problems: [
          {
            file: "package.json",
            line: 1,
            message: `The "guard" script must not end before its command body. Keep the required prefix "${ROOT_PREFIX}" and write the intended command after it.`,
          },
        ],
        failures: [],
        scanned: 1,
