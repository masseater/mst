...
writeFixture("owner/order.assets.ts", "export const rows = [1, 2];\n");
writeFixture("owner/order.test.ts", "");
writeFixture("owner/plain.ts", "export const total = 1;\n");
writeFixture("owner/relay.ts", 'export * from "./order.assets.ts";\n');
writeFixture("owner/named-relay.ts", 'export { rows } from "./order.assets.ts";\n');
writeFixture("owner/deep-relay.ts", 'export * from "./relay.ts";\n');
writeFixture(
  "owner/plain-relay.ts",
  'import { rows } from "./order.assets.ts";\nexport { rows };\n',
);
writeFixture("other/order.assets.ts", "export const rows = [3];\n");
...
writeFixture("packages/shared/src/index.ts", "export const shared = 1;\n");
writeFixture("packages/shared/src/table.assets.ts", "export const table = [4];\n");
mkdirSync(fixturePath("node_modules/@fixture"), { recursive: true });
symlinkSync(fixturePath("packages/shared"), fixturePath("node_modules/@fixture/shared"), "dir");

writeFixture(
  "node_modules/outside-pkg/package.json",
  JSON.stringify({ name: "outside-pkg", exports: { "./data": "./order.assets.ts" } }),
);
writeFixture("node_modules/outside-pkg/order.assets.ts", "export const rows = [5];\n");

writeFixture(
  "aliased/tsconfig.json",
  JSON.stringify({ compilerOptions: { baseUrl: ".", paths: { "@data/*": ["./values/*"] } } }),
);
writeFixture("aliased/values/order.assets.ts", "export const rows = [6];\n");

writeFixture("inherited/tsconfig.json", JSON.stringify({ extends: "./tsconfig.base.json" }));
writeFixture(
  "inherited/tsconfig.base.json",
  JSON.stringify({
    compilerOptions: { paths: { "@shared/values": ["../values/order.assets.ts"] } },
  }),
);
writeFixture("values/order.assets.ts", "export const rows = [7];\n");
...
describe("dont-review-it/no-cross-spec-assets-import--use-own-assets", () => {
    ...
    valid: [
      {
        ...
        filename: fixturePath("owner/order.test.ts"),
      },
      {
      ...
      },
      {
        ...
        filename: fixturePath("owner/order.assets.ts"),
      },
    ],
    invalid: [
      {
      ...
      },
      {
        ...
        filename: fixturePath("other/order.test.ts"),
        errors: [
          {
            messageId: "crossSpecAssetsImport",
            data: {
      ...
      },
      {
        ...
        code: 'import { rows } from "./order.assets.ts";',
        filename: fixturePath("owner/plain.ts"),
        errors: [
          {
      ...
      },
      {
        ...
        filename: fixturePath("aliased/reader.test.ts"),
        errors: [
          {
            messageId: "crossSpecAssetsImport",
            data: {
      ...
      },
      {
        ...
        filename: fixturePath("inherited/reader.test.ts"),
        errors: [
          {
            messageId: "crossSpecAssetsImport",
            data: {
      ...
      },
      {
        ...
        code: 'import { rows } from "./order.assets.ts";',
        filename: fixturePath("owner/checkout.spec.ts"),
  ...
  test("the options schema declares the two naming vocabularies and refuses any other key", () => {
    expect(noCrossSpecAssetsImport.meta.schema).toStrictEqual([
      {
        type: "object",
        properties: {
