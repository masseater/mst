...
describe("dont-review-it/no-default-export--use-named-export", () => {
  testLintRule(noDefaultExport, {
    valid: [
      ...
      {
        name: "the oxlint plugin entry is exempt because the tool reads it by that name",
        code: "const plugin = { meta: { name: 'x' }, rules: {} };\nexport default plugin;",
        filename: "/repo/packages/dont-review-it/src/plugin.ts",
      },
      {
        name: "a build config is exempt because the tool reads it by that name",
        code: "export default { lint: {} };",
        filename: "/repo/vite.config.ts",
      },
      {
        name: "the exemption follows the base name into any workspace",
        code: "export default { pack: {} };",
        filename: "/repo/apps/website/vite.config.ts",
      },
      {
        name: "an added name joins the exemption when the option names it",
        code: "export default { input: 'src/index.ts' };",
        filename: "/repo/rollup.config.ts",
        options: [{ toolRequiredFileNames: ["plugin.ts", "vite.config.ts", "rollup.config.ts"] }],
      },
      {
    ...
    invalid: [
      ...
      {
        name: "replacing the exempt names puts the built-in ones back under the rule",
        code: "export default { pack: {} };",
        filename: "/repo/vite.config.ts",
        options: [{ toolRequiredFileNames: ["rollup.config.ts"] }],
        errors: [{ messageId: "defaultExport" }],
      },
    ],
  });
