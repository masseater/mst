...
describe("comparedOperandsOf", () => {
  ...
  it("hands over nothing for a call that carries no matcher", () => {
    expect(spelledTypesIn("expect(report)")).toBe(null);
  });
...
describe("unwrapCopiedValue", () => {
  ...
  it("stops at a list that spreads one value beside another", () => {
    expect(unwrapCopiedValue(expressionIn("[...ids, extra]")).type).toBe("ArrayExpression");
  });

  it("stops at a list written out without a spread", () => {
    expect(unwrapCopiedValue(expressionIn('["a", "b"]')).type).toBe("ArrayExpression");
  });
