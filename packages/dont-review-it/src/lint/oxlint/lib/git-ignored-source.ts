...
import { gitRawOutput } from "./git-output.ts";
import { pathIsInside } from "./path-is-inside.ts";

export const gitIgnorePatternForLiteralPath = (repositoryPath: string): string | null => {
  if (
    ...
    return null;
  const escapedPath = repositoryPath.replaceAll(/[\\*?!#[\] ]/gu, String.raw`\$&`);
  return `/${escapedPath}`;
};
...
export const gitIgnoredRepositoryPaths = (repositoryRoot: string): ReadonlySet<string> => {
  if (!carriesRepositoryLink(repositoryRoot)) return new Set();

  const ignoredPathOutput = measureStage("canonical.scope.git", () =>
    gitRawOutput(["ls-files", "--others", "--ignored", "--exclude-standard", "--directory", "-z"], {
      cwd: repositoryRoot,
      env: process.env,
    }),
  );
...
export const readGitSourceScope = (repositoryRoot: string): GitSourceScope => {
  ...
  const ignoredPaths = gitIgnoredRepositoryPaths(root);
  return {
