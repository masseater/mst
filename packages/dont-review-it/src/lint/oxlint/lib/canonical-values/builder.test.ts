...
const writeRepositoryFile = (root: string, relativePath: string, contents: string): void => {
...
test("an annotated array declaration becomes the catalog entry for its concept", () => {
  ...
  writeRepositoryFile(
    root,
    "src/order-status.ts",
    annotate("order.status", 'export const ORDER_STATUSES = ["draft", "published"] as const;'),
  );
...
test("an annotated type alias takes the members of its union", () => {
  ...
  writeRepositoryFile(
    root,
    "src/order-status.ts",
    annotate("order.status", 'export type OrderStatus = "draft" | "published";'),
  );
...
test("a declaration the package export map reaches carries the specifier that reaches it", () => {
  ...
  writeRepositoryFile(
    ...
    annotate("order.status", 'export const ORDER_STATUSES = ["draft"] as const;'),
  );
...
test("a declaration no export map reaches carries no specifier", () => {
  ...
  writeRepositoryFile(root, "packages/vocabulary/src/index.ts", "export const marker = 1;\n");
  writeRepositoryFile(
    root,
    "packages/vocabulary/src/order-status.ts",
    annotate("order.status", 'export const ORDER_STATUSES = ["draft"] as const;'),
...
test("an annotation on a declaration that holds no literals drops the hint", () => {
  ...
  writeRepositoryFile(
    root,
    "src/order-status.ts",
    annotate("order.status", "export const ORDER_STATUSES = buildStatuses();"),
  );
...
test("the catalog is built once per repository root within a process", () => {
  ...
  writeRepositoryFile(
    root,
    "src/order-status.ts",
    annotate("article.status", 'export const ARTICLE_STATUSES = ["published"] as const;'),
  );
...
test("a changed input rebuilds the catalog without anyone clearing the cache", () => {
  ...
  writeRepositoryFile(
    root,
    "src/order-status.ts",
    annotate("order.status", 'export const ORDER_STATUSES = ["draft", "archived"] as const;'),
  );
  const catalog = buildCanonicalValuesCatalog({ repositoryRoot: root });
...
test("a workspace missing from the checkout costs only the hints that workspace owned", () => {
  ...
  writeRepositoryFile(
    root,
    "packages/kept/src/order-status.ts",
    annotate("order.status", 'export const ORDER_STATUSES = ["draft"] as const;'),
  );
  writeRepositoryFile(
    root,
    "packages/pruned/src/article-status.ts",
    annotate("article.status", 'export const ARTICLE_STATUSES = ["published"] as const;'),
  );
...
for (const { form, write } of ANNOTATION_FORMS) {
  test(`an annotation written as ${form} declares its concept`, () => {
    ...
    writeRepositoryFile(
      root,
      "src/order-status.ts",
      write("order.status", 'export const ORDER_STATUSES = ["draft", "published"] as const;'),
    );
...
test("a declaration that holds a single value is declared like any other", () => {
  ...
  writeRepositoryFile(
    root,
    "src/payment-method.ts",
    annotate("payment.method", 'export const PAYMENT_METHOD = "card";'),
  );
