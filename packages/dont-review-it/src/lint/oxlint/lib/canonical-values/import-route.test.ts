...
test("a specifier below the registered export path is registered", () => {
  expect(statusOf("@mst/order-vocabulary/status", "/repository/packages/order/src/schema.ts")).toBe(
    "registered",
  );
});

test("a specifier that only starts with the same letters as the export path is not registered", () => {
  expect(statusOf("@mst/order-vocabulary-legacy", "/repository/packages/order/src/schema.ts")).toBe(
    "external",
  );
});

test("a relative specifier that resolves to the registered declaration is registered", () => {
  expect(statusOf("./order-status.ts", "/repository/packages/order-vocabulary/src/schema.ts")).toBe(
    "registered",
  );
});
...
test("a relative specifier written with the js extension resolves to the ts declaration", () => {
  expect(statusOf("./order-status.js", "/repository/packages/order-vocabulary/src/schema.ts")).toBe(
    "registered",
  );
});
