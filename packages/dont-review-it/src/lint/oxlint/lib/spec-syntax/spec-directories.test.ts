...
import {
  DEFAULT_SPEC_DIRECTORY_NAMES,
  specDirectoryNamesFrom,
  specDirectoryOf,
} from "./spec-directories.ts";

const DEFAULT_NAMES = new Set(DEFAULT_SPEC_DIRECTORY_NAMES);

describe("spec-directories", () => {
  test("the directory names a spec directory carries by default are the ones this repository already names", () => {
    expect(DEFAULT_SPEC_DIRECTORY_NAMES).toStrictEqual([
      "__specs__",
      "__tests__",
      "spec",
      "specs",
  ...
  test("a rule run without settings reads the directory names the rule itself carries", () => {
    expect(specDirectoryNamesFrom([])).toStrictEqual(DEFAULT_NAMES);
    expect(specDirectoryNamesFrom([{}])).toStrictEqual(DEFAULT_NAMES);
  });
