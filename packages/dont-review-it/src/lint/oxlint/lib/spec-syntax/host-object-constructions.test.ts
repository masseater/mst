...
describe("host-object-constructions", () => {
  ...
  test("the roster and the runtime module list stand until the repository replaces them", () => {
    expect([...hostObjectTypesFrom([])]).toStrictEqual([...DEFAULT_HOST_OBJECT_TYPES]);
    expect([...runtimeModulesFrom([])]).toStrictEqual([...DEFAULT_RUNTIME_MODULES]);
    expect([...hostObjectTypesFrom(["error"])]).toStrictEqual([...DEFAULT_HOST_OBJECT_TYPES]);
    expect([...hostObjectTypesFrom([{ hostObjectTypes: "Response" }])]).toStrictEqual([
      ...DEFAULT_HOST_OBJECT_TYPES,
    ]);
    expect([...hostObjectTypesFrom([{ hostObjectTypes: [] }])]).toStrictEqual([
      ...DEFAULT_HOST_OBJECT_TYPES,
    ]);
    expect([...hostObjectTypesFrom([{ hostObjectTypes: [1] }])]).toStrictEqual([
      ...DEFAULT_HOST_OBJECT_TYPES,
    ]);
  });
