...
const it = test
  ...
  .extend("routesReachedFromTheOwningPackage", () =>
    importRoutesIn({
      body: parseSync(RELATIVE_PATH, `import { helper } from "./deep/helper.ts";`).program.body,
      relativePath: RELATIVE_PATH,
    }),
  )
  .extend("routesReachedFromAnotherPackage", () =>
    importRoutesIn({
      body: parseSync(
  ...
  .extend("routesOfImportlessStatement", () =>
    importRoutesIn({
...
describe("importRoutesIn", () => {
  it("routes a named import to the module it came from", ({ routesOfNamedImport }) => {
    expect(routesOfNamedImport).toStrictEqual(new Map([["readFileSync", "node:fs#readFileSync"]]));
  });
  ...
  it("routes a default import to the default entry of the module", ({ routesOfDefaultImport }) => {
