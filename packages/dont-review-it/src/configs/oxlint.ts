...
const TEST_FILE_NAME_PATTERN = String.raw`\.test\.tsx?$`;

const TEST_DIRECTORY_PATTERN = "**/{test,tests,__tests__,spec,__specs__}/**";

const ADJACENT_TEST_FILE_NAME_PATTERN =
  "place-the-test-file-next-to-its-source-instead-of-a-test-directory";
...
const TOOL_REQUIRED_FILE_NAMES = ["knip.ts", "plugin.ts", "vite.config.ts"];

const ASSIGN_ONLY_TARGETS = ["RuleTester.describe", "RuleTester.it", "RuleTester.itOnly"];

const EXTERNAL_IO_PACKAGES = ["@opentelemetry/exporter-metrics-otlp-http"];
...
export const oxlint: OxlintConfig = defineConfig({
  ...
  overrides: [
    ...
    {
      files: [TEST_DIRECTORY_PATTERN],
      rules: {
        "vitest/consistent-test-filename": [
          LINT_SEVERITY.ERROR,
          { pattern: ADJACENT_TEST_FILE_NAME_PATTERN },
        ],
      },
    },
    {
      files: ["packages/dont-review-it/src/configs/preset-lint-configs.ts"],
      rules: { "unicorn/prefer-structured-clone": LINT_SEVERITY.OFF },
    },
  ],
  rules: {
    ...UPSTREAM_RULES,
    ...
    [`${PLUGIN_NAME}/${noDefaultExport.name}`]: [
      LINT_SEVERITY.ERROR,
      { toolRequiredFileNames: [...TOOL_REQUIRED_FILE_NAMES] },
    ],
    ...
    [`${PLUGIN_NAME}/${noNonBoundaryDouble.name}`]: [
      LINT_SEVERITY.ERROR,
      { externalIoPackages: [...EXTERNAL_IO_PACKAGES] },
    ],
    ...
    [`${PLUGIN_NAME}/${noReassign.name}`]: [
      LINT_SEVERITY.ERROR,
      { assignOnlyTargets: [...ASSIGN_ONLY_TARGETS] },
    ],
    ...
    [`${PLUGIN_NAME}/no-version-range--pin-the-exact-version`]: LINT_SEVERITY.ERROR,
    ...
    "vitest/consistent-test-filename": [LINT_SEVERITY.ERROR, { pattern: TEST_FILE_NAME_PATTERN }],
  },
});
