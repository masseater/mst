...
- A `./vitest` test-support entry that exports `standardIoTest` for isolated standard-stream capture and `runAsyncProcess` for subprocesses bounded by a wall-clock deadline with process-tree termination.
- A `check` CLI that runs every check the lint toolchain cannot express: canonical values, duplicated bodies, workflow definitions, dependency declarations, preset adoption, entry composition, and the packaging of shipped agent skills. Its `--write` option regenerates owned entry scripts and shipped skill versions before checking them.
