...
4 層目は、メトリクスを 1 つだけ書く形が最も静かに壊れる点である。`lines` だけ 100 にした設定は一見厳しいが、`branches` が無いので分岐は 1 本も通っていなくても構わない。行カバレッジは分岐の存在を見ないため、この組み合わせでは「全行を通ったが片方の分岐しか実行していない」コードが満点になる。4 つを揃えて要求するのは、どれか 1 つが緩いと残り 3 つの意味が薄れるからである。

## どう直すか

設定ファイルに `test.coverage.thresholds` を書き、4 メトリクスすべてに要求値を入れる。全メトリクスが 100 でよいなら `thresholds: { 100: true }` と書けば同じことになる。
