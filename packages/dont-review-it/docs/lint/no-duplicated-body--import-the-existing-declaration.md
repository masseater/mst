...
見るのはトップレベルの宣言だけで、対象は 2 つの形。

- 束縛（`const` / `let` / `var`）。宣言した名前の直後から、その文の終わりまでを本体とする。型注釈も初期化子も本体に含む
- 関数宣言。宣言した名前の直後から、対応する閉じ括弧までを本体とする

**宣言そのものの名前は本体に含めない。** 名前を変えただけの複製を捕まえるため。実際に、同じ glob マッチャが 2 つのパッケージで別々の名前を持って存在していた。

呼び出し位置に直接書かれた無名の関数式は対象外。他の宣言の内側にある宣言も対象外。トップレベルに立っていないものは、その場の手続きの一部であって、リポジトリのどこかから import できる単位ではない。

### 一致の判定

本体をトークン列に直してから比べる。トークン列にするときに落ちるもの、残るものは次のとおり。
...
トークンが 12 個に満たない本体は索引に入れない。短い本体は偶然に一致する。`= 1` や `= []` が repository 中で一致しても、そこには共有すべき振る舞いが無い。
...
- 一度しか使われていない宣言。判定は決定的にできるが、規範として成立しない。このリポジトリの非 export のトップレベル宣言 204 件のうち 140 件が 1 回しか参照されておらず、その大半は長い手続きを名前付きの段に割る良い書き方である。export を 1 箇所からしか参照されないものに絞っても 116 件中 58 件が該当する。数えたうえで、ルールにしないと決めた
