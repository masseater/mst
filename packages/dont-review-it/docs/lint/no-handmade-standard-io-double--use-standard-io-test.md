...
テストファイル（`*.test.ts` / `*.test.tsx`）が stdout / stderr のテストダブルを自前で組み立てている形を検出する。系統は 3 つある。

1. `*.extend({ ... })` のフィクスチャ定義に `stdout` / `stderr` という名前を宣言する形。共有フィクスチャを import しているファイルでも、宣言し直した時点で報告される
