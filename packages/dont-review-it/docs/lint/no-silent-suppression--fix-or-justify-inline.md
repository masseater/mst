...
**リンタの設定。** ファイル名が `vite.config` で始まり拡張子が `.ts` / `.mts` / `.cts` / `.js` / `.mjs` / `.cjs` のファイルだけを、設定として追加で読む。default export された式まで降り、それが関数呼び出しで包まれていれば第 1 引数まで降りる。`lint` の値も同じように包みを外す。`withGitExcludes` で包まれた設定は、この経路で中身が見える。
