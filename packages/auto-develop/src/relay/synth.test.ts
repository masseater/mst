...
const it = test
  ...
  .extend("baseUpdateCheckId", () => baseUpdateCheckDeliveryId(behindPull));
...
describe("synthesizeEnvelope", () => {
  ...
  it("作者アカウントが削除済みなら user を付けない", ({ envelopeWithoutAuthorLogin }) => {
    expect(envelopeWithoutAuthorLogin.payload).toStrictEqual({
      action: "synchronize",
  ...
  it("pull_request を本来持たない check_suite にはルーティング専用に付加される", ({
    ...
    expect(checkSuiteEnvelope.payload).toStrictEqual({
      action: "completed",
