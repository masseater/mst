...
const it = test
  ...
  .extend("pathWithoutAnyHint", () => resolveSnapshotPath({ explicitPath: undefined, env: {} }))
...
describe("resolveSnapshotPath", () => {
  ...
  it("どちらも無ければリポジトリルート配下の既定パスに落ちる", ({ pathWithoutAnyHint }) => {
    expect(pathWithoutAnyHint).toMatch(/logs\/auto-develop-queue\.json$/);
  });
...
describe("createSnapshotWriter", () => {
  ...
  it("再試行も失敗したらエラーログを残してメモリ運転を続ける", ({ blockedWriteErrorCalls }) => {
    expect(blockedWriteErrorCalls.length).toStrictEqual(1);
  });
});
