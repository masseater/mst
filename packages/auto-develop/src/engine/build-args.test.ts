...
describe("buildClaudeArgs", () => {
  describe("バイパス有効", () => {
    const it = test.extend("claudeArgsWithBypass", () =>
      buildClaudeArgs({ prompt: "review this", prNumber: 7, bypassPermissions: true }));

    it("完全形の引数列になる", ({ claudeArgsWithBypass }) => {
  ...
  describe("バイパス無効", () => {
    const it = test.extend("claudeArgsWithoutBypass", () =>
      buildClaudeArgs({ prompt: "review this", prNumber: 7, bypassPermissions: false }));

    it("permission-mode auto になる", ({ claudeArgsWithoutBypass }) => {
...
describe("buildCodexArgs", () => {
  describe("バイパス有効", () => {
    const it = test.extend("codexArgsWithBypass", () =>
      buildCodexArgs({
        prompt: "fix this",
        cwd: "/work/pr-1",
        repoRoot: "/repo",
    ...
    it("承認ポリシーが exec の前、全権フラグが exec の後になる", ({ codexArgsWithBypass }) => {
  ...
  describe("バイパス無効", () => {
    const it = test.extend("codexArgsWithoutBypass", () =>
      buildCodexArgs({
        prompt: "fix this",
        cwd: "/work/pr-1",
        repoRoot: null,
    ...
    it("on-request と自動レビュアー設定になる", ({ codexArgsWithoutBypass }) => {
