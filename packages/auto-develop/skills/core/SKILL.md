...
The image runs from `/repository`. Mount a self-contained Git checkout at
that path and mount a writable directory at `/var/log/auto-develop`. A linked
worktree whose `.git` file points outside the mount is not self-contained.
The image includes the pinned Claude Code CLI. Supply its authentication through
the environment or mount its authenticated configuration into the container.
