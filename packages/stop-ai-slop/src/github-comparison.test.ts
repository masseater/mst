...
describe("compareGitHubPullRequest", () => {
  it("refuses a compare whose changed-file inventory may be truncated", async () => {
    await expect(
    ...
    ).rejects.toThrow("may be truncated at GitHub's 300-file comparison limit");
  });

  it("refuses a compare answered without a changed-file inventory", async () => {
    await expect(
      compareGitHubPullRequest({
        repositoryRoot: "/checkout",
        repository: "owner/name",
    ...
    ).rejects.toThrow("without a changed-file inventory");
  });
  ...
  it("encodes every repository path segment before requesting its contents", async () => {
    const request = vi.fn<GitHubRequest>(async (path) =>
      path.startsWith("/repos/owner/name/compare/")
        ...
        : { content: encoded("export const value = true;\n") },
    );
    const comparison = await compareGitHubPullRequest({
      repositoryRoot: "/checkout",
      repository: "owner/name",
    ...
    expect(comparison.files[0]?.afterPath).toBe("src/hash#query?percent%/日本語.ts");
    expect(request).toHaveBeenNthCalledWith(1, "/repos/owner/name/compare/basetip...headsha");
    expect(request).toHaveBeenNthCalledWith(
      2,
      "/repos/owner/name/contents/src/hash%23query%3Fpercent%25/%E6%97%A5%E6%9C%AC%E8%AA%9E.ts?ref=headsha",
    );
  });
