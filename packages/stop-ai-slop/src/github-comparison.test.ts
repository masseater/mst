...
describe("compareGitHubPullRequest", () => {
    ...
    const it = test.extend("apiSideComparison", () =>
      compareGitHubPullRequest({
        ...
        request: async (path: string) => {
          ...
          return {
            content: Buffer.from(writtenContent, "utf8").toString("base64"),
            encoding: "base64",
          };
        },
      }));
    ...
    const it = test.extend("undecodablePreviousSourceComparison", () =>
      compareGitHubPullRequest({
        ...
        request: async (path: string) => {
          ...
          const writtenContent = writtenContents[path];
          if (writtenContent === undefined) throw new Error(`unexpected request ${path}`);
          return { content: Buffer.from(writtenContent).toString("base64"), encoding: "base64" };
        },
      }));
      ...
      const [failure] = await attemptAsync<unknown, Error>(() =>
        compareGitHubPullRequest({
          ...
          request: async (path: string) => {
            ...
            if (writtenContent === undefined) throw new Error(`unexpected request ${path}`);
            return {
              content: Buffer.from(writtenContent).toString("base64"),
              encoding: "base64",
            };
          },
        }),
  ...
  describe("a file the contents API answered without base64 content", () => {
    const it = test.extend("failureFromReadingAFileWithoutBase64Content", async () => {
      const [failure] = await attemptAsync<unknown, Error>(() =>
        compareGitHubPullRequest({
          repositoryRoot: "/checkout",
          repository: "owner/name",
    ...
    it("refuses a file the contents API answered without base64 content", ({
      failureFromReadingAFileWithoutBase64Content,
    }) => {
      expect(failureFromReadingAFileWithoutBase64Content).toBe(
        "Do not read a file the contents API answered with none encoding.",
      );
    });
  });
  ...
  describe("a compare whose changed-file inventory may be truncated", () => {
    const it = test.extend("truncatedInventoryFailure", async () => {
      const [failure] = await attemptAsync<unknown, Error>(() =>
        compareGitHubPullRequest({
          repositoryRoot: "/checkout",
          repository: "owner/name",
    ...
    it("refuses to read past GitHub's comparison limit", ({ truncatedInventoryFailure }) => {
      expect(truncatedInventoryFailure).toBe(
        "Do not read past a GitHub comparison whose changed-file inventory may be truncated at GitHub's 300-file comparison limit.",
      );
    });
  });

  describe("a compare answered without a changed-file inventory", () => {
    const it = test.extend("missingInventoryFailure", async () => {
      const [failure] = await attemptAsync<unknown, Error>(() =>
        compareGitHubPullRequest({
          repositoryRoot: "/checkout",
          repository: "owner/name",
    ...
    it("refuses the incomplete answer", ({ missingInventoryFailure }) => {
      expect(missingInventoryFailure).toBe(
        "Do not read a GitHub comparison answered without a changed-file inventory.",
      );
    });
  });

  describe("a repository path and revisions containing reserved characters", () => {
    const it = test.extend("encodedPathComparison", () =>
      compareGitHubPullRequest({
        repositoryRoot: "/checkout",
        repository: "owner/name",
        baseRevision: "base tip",
        headRevision: "heads/feature#one?",
        request: async (path: string) => {
          if (path === "/repos/owner/name/compare/base%20tip...heads%2Ffeature%23one%3F") {
            return {
              merge_base_commit: { sha: "base sha" },
              files: [
                {
                  filename: "src/hash#query?percent%/日本語.ts",
                  status: "added",
          ...
          if (
            path ===
            "/repos/owner/name/contents/src/hash%23query%3Fpercent%25/%E6%97%A5%E6%9C%AC%E8%AA%9E.ts?ref=heads%2Ffeature%23one%3F"
          ) {
            return {
              content: Buffer.from("export const value = true;\n", "utf8").toString("base64"),
              encoding: "base64",
            };
          }
    ...
    it("encodes each path segment and each revision", ({ encodedPathComparison }) => {
      expect(encodedPathComparison).toStrictEqual({
        repositoryRoot: "/checkout",
        ...
        files: [
          {
            kind: "added",
            beforePath: null,
            afterPath: "src/hash#query?percent%/日本語.ts",
            beforeSource: null,
            afterSource: "export const value = true;\n",
            addedLines: [1],
            firstAddedLine: 1,
          },
        ],
  ...
  describe("a pull request that changed nothing", () => {
    const it = test.extend("emptyComparison", () =>
      compareGitHubPullRequest({
        ...
        baseRevision: "basetip",
        headRevision: "headsha",
        request: async () => ({ merge_base_commit: { sha: "basesha" }, files: [] }),
      }));
