...
const removedFilesIn = (files: readonly ComparisonFile[]): readonly RemovedFile[] =>
  files.flatMap((file) => {
    if (file.kind !== "deleted" || !isImplementationSourceFile(file.beforePath)) return [];
...
const removedExportsIn = (files: readonly ComparisonFile[]): readonly RemovedExport[] =>
  files.flatMap((file) => {
    if (
      file.kind !== "changed" ||
      !isImplementationSourceFile(file.beforePath) ||
...
const sourceFor = (
  ...
  return path !== null && source !== null && isVerificationTestFile(path)
...
const correspondingTestProblems = (
  ...
  const deletedPathByTestPath = new Map(
    removedFiles.map(({ path }) => [correspondingTestPathFor(path), path]),
  );
  return files
