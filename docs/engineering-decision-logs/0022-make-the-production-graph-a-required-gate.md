...
あわせて、ゲートの定義を 1 つにし、名前を `guard` にする。`pre-commit` / `pre-merge-commit` / `pre-push` / `post-merge` の 4 フックと CI は、どれも `vp run guard` だけを呼ぶ。定義が 1 箇所になるので、ゲートに 1 段足したときに 5 箇所を揃える作業が消え、「手元は通ったのに CI で落ちる」も「CI は通るのに push できない」も起きなくなる。pre-push は `vp check` だけの頃より遅くなるが、押した後で気づく取りこぼしと引き換えである。

規範文書の検査も `guard` に畳む。`docs` と `docs:write` を別のスクリプトとして残すと、ゲートが呼ぶものと人が手で呼ぶものが別々に育つ。自動で直せる側は `guard:fix` にまとめ、`vp check --fix` と生成部分の更新を 1 つの入口にする。
