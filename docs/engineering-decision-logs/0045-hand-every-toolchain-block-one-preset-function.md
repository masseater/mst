# 0045. ツールチェーンの各ブロックに preset の関数を 1 つずつ渡す
...
`@mst/dont-review-it` は `oxlint`、`withGitExcludes`、そして [0044](0044-let-the-formatter-own-where-markdown-lines-break.md) が足した整形の選択という 3 つのものを公開面に並べていた。利用者の `vite.config.ts` は、`lint` に `withGitExcludes({ extends: [oxlint, ...] })` を書き、`fmt` に `withGitExcludes({ ...整形の選択 })` を書く。どれをどのブロックへどう組むかは呼び出し側の作業であり、組み方を間違えても lint は緑のまま通る。
