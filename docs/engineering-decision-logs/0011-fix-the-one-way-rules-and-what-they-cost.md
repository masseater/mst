...
**例外はルール本体ではなく設定側の対象指定だけに置く。** ルールに抑制の口を作らないという設計を守ったまま緑にするには、この形しかない。mst で対象から外したのは 3 か所で、理由はそれぞれ違う。

- `apps/website/src/**` — Vite+ のスキャフォールド生成物。カウンタの状態はイベントハンドラをまたいで保つもので、不変条件文書が「管理された可変境界」として未決にしている領域そのものである。かつ mst の規約はスキャフォールド生成物を独自設計に置き換えないと定めている
- `packages/dont-review-it/src/cli.ts` — `process.exitCode` への代入。プロセスの終了コードを module スコープの入口から返す手段は他に無い。`process.exit` は書き込みの完了を待たない
- `packages/lint-rule-authoring/src/rule-tester.ts` — oxlint の `RuleTester` が `describe` / `it` を static setter でしか受け取らない

後ろの 2 つは、不変条件文書が「代入でしか操作できないプラットフォーム API」として名指ししている形である。文書はそれ自体は抑制の根拠にならないと書いているので、抑制ではなく適用対象の指定として扱った。どのファイルが外れているかはルート `vite.config.ts` を読めば分かる。
...
例外に入れた 3 か所は、ルールが黙っている場所であって、書き方が認められた場所ではない。増やすときは同じ水準の理由が要る。
