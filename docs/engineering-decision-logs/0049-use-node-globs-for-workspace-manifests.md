# 0049. workspace manifest の列挙に Node.js の glob を使う
...
同じ manifest 列挙を test command と entry composition の検査にも使うと、読み飛ばした package が coverage 設定を CLI で上書きしたり、必須の実行ラッパーを外したりしても検査対象に入らない。これは誤報を避ける境界ではなく、main の gate を狭める迂回経路になる。
...
**catalog、test command、entry composition は同じ列挙結果を使う。** 同じ workspace 定義を読む検査の間で、対象 package の集合を分岐させない。
...
- nested workspace の catalog 宣言、test command、entry composition も検査対象に入る
