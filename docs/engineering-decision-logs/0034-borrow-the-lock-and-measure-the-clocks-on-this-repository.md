...
- 待機予算 1900 秒。`guard` が持つ 1800 秒の打ち切り、強制終了までの猶予、子 process の終了と次の poll を含み、先行 1 本が決着するまで次の競合者を queue に保つ値。
...
- 同じホストで `guard` が 2 本重なった場合、後発の外側所要は待機 1900 秒と自身の打ち切り 1800 秒、強制終了の猶予 5 秒を合わせて最大約 61 分 45 秒となる。これは先行を失敗させず完走させてから後発を実行するための上限である。
