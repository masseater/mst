# 0051. ダブルの隔離は各テスト設定に宣言させ、要求する値はルールが持つ
...
`dont-review-it` は `lint-rule-authoring` / `repository-checks` / `verified-specifications` を実行時依存に持ち、全パッケージは `spool` / `throttle` のために `ai-native` を devDependency に持つ。したがって `dont-review-it` は、自分の依存先にも依存元にも設定を配れない。pnpm の install は通り、詰まるのはタスクの計画だけである。
