# 0032. Ponytail のうち違反状態を決定できる部分だけを強制する
...
[強制の機構](../guidelines/enforcement.md) は、機械で一意に判定できる違反にガードを置き、破られた不変条件と修正の方向を報告することを求める。[0030](0030-separate-the-violation-from-the-choice-of-repair.md) は、修正案が複数あることを理由に違反を成功へ読み替えない。
...
**local な単一実装 contract を `no-single-use-local-type--inline-at-the-use-site` が検出する。** 非 export のトップレベル type/interface の resolved binding が 1 箇所以下からしか型として参照されない場合に限る。唯一の `implements`、唯一の interface heritage、通常の型参照、自己参照、未使用を分けて修正方向を報告する。export、declaration file、declaration merging、2 箇所以上が合意する型は対象にしない。

**実質固定された local option を `no-invariant-default-parameter--remove-unused-option` が検出する。** 外へ escape しない function の全 callsite を数え、既定値を含む effective argument が同じ静的値だけになる parameter に限る。値を本体へ埋め込み、parameter と全 actual argument を削除する方向を報告する。外部 callsite、動的な値、実行 context の観測、overload contract が残る function は対象にしない。
