...
- 汎用ユーティリティ（配列・オブジェクト・文字列の操作、debounce / throttle、deep clone / deep merge など）は [es-toolkit](https://github.com/toss/es-toolkit) を使う。同等の処理を自前実装せず、`es-toolkit` の関数を包み直すだけのラッパーも作らない
